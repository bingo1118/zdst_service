...
<div class="title_tj">
 <a class="layui-btn layui-btn-small" style="background:#3b66cc;" href="queryItems_test.do">返回</a> 
	...
	<div class="contain_tj">
        <div class="conten_tj">
        	<div class="conten_tj_tit">用户绑定主机状态推送</div>
            <div class="conten_txt">
            	<form id="add">
                    ...
                    <label>主机MAC编号：<input type="text" name="smoke" id="smoke"><input type="button" value="添加" id="new" style="width: 50px; margin-left: 15px;cursor:pointer;"/><span><b style="color:#e90000;">*</b> 请填写正确的烟感MAC编号</span></label>
                    <p class="smoke">已绑定的主机：</p>
...
var username = $("#username").val();
	dwr.engine.setAsync(false);
   		pushMessageCompont.getRepeater(username, function(data) {
		if(data!==null){
			$(".deviceTableRecord .deviceTableRecordli").empty();
				var json = eval(data);
				var deviceTableRecordStr = "";
...
$(".deviceTableRecord").on("click","a.delete",function(){
	...
	var username = $("#username").val();
	dwr.engine.setAsync(false);
   		pushMessageCompont.delRepeater(username,devMac, function(data) {
		 if(data!==null){
		 alert("解绑成功");
			$(".deviceTableRecord .deviceTableRecordli").empty();
				var json = eval(data);
