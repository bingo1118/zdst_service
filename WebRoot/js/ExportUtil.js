...
$(function(){
			...
			var devStateLength = $(".devState").length;
			for(var i=0;i<devStateLength;i++){
				var devStateVal = $(".devState:eq("+i+")").attr("val");
				if(devStateVal=="在线"){
					$(".devState[val='在线']").css("color","green");
				}else if(devStateVal=="掉线"){
					$(".devState[val='掉线']").css("color","red");
				};
			}
			$("#daochu").click(function(){
				...
				$.ajax({
					type:"GET",
					url:"exportExcel.do?methodName=allDevice",
					dataType:"json",
					success:function(){
						console.log("请求成功。");
						alert("导出成功，在C:/excel/文件目录下");
					},
					error:function(){
					}
				});
	...
	var Export = {
		...
		__ieExport : function(datas){
			...
			if(typeof datas === 'string'){
				...
				try{
					sel.select();  //there ie10銆�1 will be error, i don't know why, but also can export 
				} catch(e){}
