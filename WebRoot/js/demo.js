...
function clickgetip(){
	var ips = clickgetip1();
	if(ips == null) ips = "未找到相关信息。";
	showOPInfo(ips);
}

function clickgetip1() {
	...
	$.ajax({
		url:"clickgetip.do?clickgetip="+szIP,
		type:"GET",
		dataType:"json",
		success:function(data){
			...
			for(var i = 0;i<json.length;i++){
				strTd = json[i];
				var iRet = WebVideoCtrl.I_Login(strTd, 1, szPort, szUsername, szPassword, {
					success: function (xmlDoc) {
						showOPInfo(strTd + "获取成功！");
						strTd = strTd;
